...
- [Getting Started](#getting-started)
  ...
  - [Calling the API](#calling-the-api)
    ...
    - [Calling Other API Endpoints](#calling-other-api-endpoints)
  - [Retries](#retries)
...
The API Executor allows you to call arbitrary OpenFGA API endpoints that may not have direct SDK methods yet, while automatically leveraging the SDK's authentication, retry logic, and error handling.

##### Example: Custom Check Request
...
var request = ApiExecutorRequestBuilder
    ...
    .PathParam("store_id", fgaClient.StoreId)
...
var response = await fgaClient.ApiExecutor().SendAsync<CheckResponse>(request);
Console.WriteLine($"Status: {response.StatusCode}");
Console.WriteLine($"Allowed: {response.Data.Allowed}");

// Or get raw JSON response
var rawResponse = await fgaClient.ApiExecutor().SendAsync(request);
Console.WriteLine($"Raw JSON: {rawResponse.RawResponse}");
```

##### Features

- **Automatic Authentication**: Uses configured credentials (API token or OAuth2)
- **Retry Logic**: Automatically retries on 429 and 5xx errors
- **Error Handling**: Throws appropriate `FgaApiError` exceptions
- **Flexible Response**: Get typed data or raw JSON
- **Custom Headers**: Add request-specific headers for debugging or tracing

##### Available Methods

- `ApiExecutor().SendAsync<T>(request)` - Returns typed response with deserialized data
- `ApiExecutor().SendAsync(request)` - Returns raw JSON string as data

##### Response Properties

- `StatusCode` - HTTP status code
- `Headers` - Response headers (case-insensitive)
- `RawResponse` - Raw response body as string
- `Data` - Deserialized response data (type varies)

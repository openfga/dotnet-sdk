...
public class ApiException : Exception {
    ...
    public static async Task<ApiException> CreateSpecificExceptionAsync(HttpResponseMessage response, HttpRequestMessage request,
        ...
        switch ((int)response.StatusCode) {
            case 401:
            case 403:
                return await OpenFgaAuthenticationError.CreateAsync(response, request, apiName).ConfigureAwait(false);
            case 400:
            case 442:
                return await OpenFgaApiValidationError.CreateAsync(response, request, apiName).ConfigureAwait(false);
            case 429:
                return await OpenFgaApiRateLimitExceededError.CreateAsync(response, request, apiName).ConfigureAwait(false);
            case 500:
                return await OpenFgaApiInternalError.CreateAsync(response, request, apiName).ConfigureAwait(false);
            default:
                return await OpenFgaApiError.CreateAsync(response, request, apiName).ConfigureAwait(false);
        }
    }
}
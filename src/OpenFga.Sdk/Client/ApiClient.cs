...
public class ApiClient : IDisposable {
    ...
    /// <returns></returns>
    /// <exception cref="OpenFgaAuthenticationError"></exception>
    public async Task<T> SendRequestAsync<T>(RequestBuilder requestBuilder, string apiName,
        ...
        if (_oauth2Client != null) {
            try {
                ...
                throw new OpenFgaAuthenticationError("Invalid Client Credentials", apiName, e);
            }
        }
    ...
    private async Task<TResult> Retry<TResult>(Func<Task<TResult>> retryable) {
        ...
        while (true) {
            ...
            catch (OpenFgaApiRateLimitExceededError err) {

...
public class OpenFgaClient : IDisposable, IOpenFgaClient {
    ...
    public OpenFgaClient(ClientConfiguration configuration, HttpClient? httpClient = null)
    {
    ...
    private string? GetAuthorizationModelId(AuthorizationModelIdOptions? options)
    {
        ...
        if (
            ...
            && !ClientConfiguration.IsWellFormedUlidString(authorizationModelId)
        )
        {
    ...
    private static WriteRequestWrites.OnDuplicateEnum MapOnDuplicateWrites(
        OnDuplicateWrites? behavior
    )
    {
        if (behavior == null)
            return WriteRequestWrites.OnDuplicateEnum.Error;
    ...
    {
        if (behavior == null)
            return WriteRequestDeletes.OnMissingEnum.Error;
    ...
    public async Task<ReadAuthorizationModelsResponse> ReadAuthorizationModels(
        ...
        await api.ReadAuthorizationModels(
            GetStoreId(options),
            options?.PageSize,
            options?.ContinuationToken,
            options,
    ...
    )
    {
        ...
        return await api.ReadAuthorizationModel(
            GetStoreId(options),
            authorizationModelId,
            options,
            cancellationToken
    ...
    {
        var response = await ReadAuthorizationModels(
        ...
        {
            return new ReadAuthorizationModelResponse
            {
                AuthorizationModel = response.AuthorizationModels?[0],
            };
        }
    ...
    {
        var maxPerChunk = options?.Transaction?.MaxPerChunk ?? 1; // 1 has to be the default otherwise the chunks will be sent in transactions
        var maxParallelReqs =
            options?.Transaction?.MaxParallelRequests
            ?? FgaConstants.ClientMaxMethodParallelRequests;
        var authorizationModelId = GetAuthorizationModelId(options);

        if (options?.Transaction?.Disable != true)
        {
            var requestBody = new WriteRequest() { AuthorizationModelId = authorizationModelId };
            ...
            {
                Writes =
                    body.Writes?.ConvertAll(tupleKey => new ClientWriteSingleResponse
                    {
                        TupleKey = tupleKey.ToTupleKey(),
                        Status = ClientWriteStatus.SUCCESS,
                    }) ?? new List<ClientWriteSingleResponse>(),
                Deletes =
                    body.Deletes?.ConvertAll(tupleKey => new ClientWriteSingleResponse
                    {
                        TupleKey = tupleKey.ToTupleKey(),
                        Status = ClientWriteStatus.SUCCESS,
                    }) ?? new List<ClientWriteSingleResponse>(),
        ...
        var deleteChunks =
            body.Deletes?.Chunk(maxPerChunk).ToList()
            ?? new List<ClientTupleKeyWithoutCondition[]>();
        ...
        return new ClientWriteResponse
        {
            Writes = writeResponses.ToList(),
            Deletes = deleteResponses.ToList(),
        };
    }
    ...
    public async Task<CheckResponse> Check(
        ...
        await api.Check(
            ...
            new CheckRequest
            {
                ...
                {
                    TupleKeys =
                        body.ContextualTuples?.ConvertAll(tupleKey => tupleKey.ToTupleKey())
                        ?? new List<TupleKey>(),
                },
    ...
    )
    {
        ...
        var maxParallelReqs =
            options?.MaxParallelRequests ?? FgaConstants.ClientMaxMethodParallelRequests;

        await ProcessCheckRequestsAsync(
            body,
            responses,
            options,
            maxParallelReqs,
        ...
        foreach (var check in body.Checks)
        {
            ...
            if (correlationIdToCheck.ContainsKey(check.CorrelationId))
            {
                throw new FgaValidationError(
                    $"Duplicate correlation ID found: {check.CorrelationId}. Correlation IDs must be unique."
                );
            }
            ...
            transformedChecks.Add(
                Utils.ClientUtils.TransformToBatchCheckItem(check, check.CorrelationId)
            );
        }
    ...
    )
    {
        ...
        var batchCheckResponse = await ClientBatchCheck(
            batchCheckRequests,
            options,
            cancellationToken
        );
        ...
        foreach (var batchCheckSingleResponse in batchCheckResponse.Responses)
        {
            ...
            if (
                batchCheckSingleResponse.Allowed
                && batchCheckSingleResponse.Request?.Relation != null
            )
            {
    ...
    )
    {
        ...
        return await api.ReadAssertions(
            GetStoreId(options),
            authorizationModelId,
            options,
            cancellationToken
    ...
    )
    {
        ...
        await api.WriteAssertions(
            GetStoreId(options),
            authorizationModelId,
            new WriteAssertionsRequest { Assertions = assertions },
            options,
            cancellationToken
        );
    }

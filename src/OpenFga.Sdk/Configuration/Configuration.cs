...
public class Configuration {
    ...
    /// </summary>
    /// <exception cref="OpenFgaRequiredParamError"></exception>
    public void IsValid() {
        if (string.IsNullOrWhiteSpace(StoreId)) {
            throw new OpenFgaRequiredParamError("Configuration", nameof(StoreId));
        }

        if (string.IsNullOrWhiteSpace(ApiScheme)) {
            throw new OpenFgaRequiredParamError("Configuration", nameof(ApiScheme));
        }

        if (string.IsNullOrWhiteSpace(ApiHost)) {
            throw new OpenFgaRequiredParamError("Configuration", nameof(ApiHost));
        }
        ...
        if (!string.IsNullOrWhiteSpace(ClientId) || !string.IsNullOrWhiteSpace(ClientSecret)) {
            if (string.IsNullOrWhiteSpace(ClientId)) {
                throw new OpenFgaRequiredParamError("Configuration", nameof(ClientId));
            }

            if (string.IsNullOrWhiteSpace(ClientSecret)) {
                throw new OpenFgaRequiredParamError("Configuration", nameof(ClientSecret));
            }

            if (string.IsNullOrWhiteSpace(ApiTokenIssuer)) {
                throw new OpenFgaRequiredParamError("Configuration", nameof(ApiTokenIssuer));
            }

            if (string.IsNullOrWhiteSpace(ApiAudience)) {
                throw new OpenFgaRequiredParamError("Configuration", nameof(ApiAudience));
            }
        }
        ...
        if (MaxRetry > 5) {
            throw new OpenFgaValidationError("Configuration.MaxRetry exceeds maximum allowed limit of 5");
        }
    }
